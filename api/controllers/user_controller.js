...
module.exports = {
    ...
    recovery: (req, res) => {
        ...
        db.query(sql, [key_recovery, req.params.username], (err, response) => {
            if (err) throw err
            sendMail_recovery(req.params.username, key_recovery).then(() => {
                ok = 1
            }).catch((error) => {
                ok = 0
            }).finally(() => {
...
function sendMail_recovery(to, key_recovery) {
    // Những thông tin dưới đây các bạn có thể ném nó vào biến môi trường env nhé.
    // Vì để demo nên mình để các biến const ở đây.
    const adminEmail = 'tool264sp@gmail.com'
    const adminPassword = '#@*7LRmrfNZcn*@)'
    ...
    const options = {
        ...
        to: to, // địa chỉ gửi đến
        subject: 'Đăng ký thành công tool264.com', // Tiêu đề của mail
        html: `<h3>Chao mừng bạn đã đăng ký tool264.com</h3><div>Bấm vào link để Thay đổi mật khẩu: </div><a target="_blank" href="https://tool264.com/recovery_save?key=${key_recovery}">Kích hoạt</a>` // Phần nội dung mail mình sẽ dùng html thay vì thuần văn bản thông thường.
    }
