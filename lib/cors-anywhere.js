...
function getHandler(options, proxy) {
  ...
  return function (req, res) {
    ...
    if (req.url.indexOf('https:/') > -1) {
      var next_char = req.url[req.url.indexOf('https:/') + 7];
      if (next_char != '/') {
        var str = req.url.toString();
        req.url = str.replace('https:/', 'https://');
      } else {
        if (req.url[req.url.indexOf('https:/') + 8] == '/') {
          if (req.url[req.url.indexOf('https:/') + 9] != '/') {
            var str = req.url.toString();
            req.url = str.replace('https:///', 'https://');
          }
        }
    ...
    if (req.url.indexOf('http:/') > -1) {
      var next_char = req.url[req.url.indexOf('http:/') + 6];
      if (next_char != '/') {
        var str = req.url.toString();
        req.url = str.replace('http:/', 'http://');
      } else {
        if (req.url[req.url.indexOf('http:/') + 7] == '/') {
          if (req.url[req.url.indexOf('http:/') + 8] != '/') {
            var str = req.url.toString();
            req.url = str.replace('http:///', 'http://');
          }
        }
